...
def reg():
    ...
    for i in details:
      if i == "department: ":
          print("department: "); dept = ["Frontend eng", "Backend eng", "UI/UX", "Data analysis", "Data science", "Project mgt", "Graphics design"]; ab = 0
          for i in dept:
...
def vote():
    ...
    if details[8] < 1:
        ...
        count +=1; sql = "UPDATE studentv SET vote = %s WHERE voter_id = %s"; mycursor.execute(sql, (count, Id)); mydb.commit(); homepage()
    else:
        print("You have voted."); homepage()
def checkresult():
    decision = input("\n1. BY DEPARTMENT          2. TOTAL     \n>>>>"); dept = ["Frontend eng", "Backend eng", "UI/UX", "Data analysis", "Data science", "Project mgt", "Graphics design"]
    if decision == "1":
        for i in dept:
